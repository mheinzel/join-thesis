...
While the \actormodel is not originally defined as a process calculus,
...
targeted to $x$ containing a single name $y$. %TODO: call x "subject"?
...
\TODO{3cm}{%
  The idea is to track each actor's name as a type and prevent it\\
  from receiving on any other channels.\\
  As this is too restrictive, the persistence property is slightly relaxed\\
  to allow temporally assuming a new name.\\
  This leads to the definition of $\ch()$.\\
}

\TODO{2cm}{%
  intuitively:\\
  ch(x): conf of one recipient x\\
  ch(t,x): conf of one recipient x, temporarily assuming the name t\\
  ?
}

% TODO: definitions like tilde and hat, tuple to set.
...
\TODO{3cm}{explain rules\\
  how are actor properties enforced?}
